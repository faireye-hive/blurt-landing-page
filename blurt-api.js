...
export async function fetchWitnessSchedule() {
  try {
    ...
    blockchainCache.witnessSchedule = 21;
    return 21;
  } catch (error) {
    ...
    case "custom_json":
      return [
        "custom_json",
        {
          ...
          json: JSON.stringify([
            "follow",
            { follower: fakeUser(), following: fakeUser() },
          ]),
        },
      ];
...
export async function fetchMarketData() {
  try {
    ...
    const resultado = marketpools.find(
      (item) => item.tokenPair === "SWAP.BLURT:SWAP.USDT"
    );
